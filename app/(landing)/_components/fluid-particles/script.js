...
const config = {
  logoLight: "/logo_vector_dark.png",
  logoSize: 1000,
  canvasBg: "#1f1f1f",
...
function updatePhysics() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    particle.velocityX *= config.damping;
    particle.velocityY *= config.damping;

    const targetX =
      currentX +
      particle.velocityX +
      (particle.originalX - currentX) +
      config.returnForce;
    const targetY =
      currentY +
      particle.velocityY +
      (particle.originalY - currentY) +
      config.returnForce;
  ...
  window.addEventListener("resize", () => {
    ...
    if (particles.length > 0) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        const col = i % dim;
        const repositonX = centerX + (col - dim / 2) * 1.0;
        const repositonY = centerY + (row - dim / 2) * 1.0;

        particles[i].originalX = repositonX;
        particles[i].originalY = repositonY;
        positionArray[i * 2] = repositonX;
        positionArray[i * 2 + 1] = repositonY;
      }
...
export default function initParticles(theme) {
  setupCanvas();
  setupWebGL();
  setupShaders();
  loadLogo(theme);
